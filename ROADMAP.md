...
| --- | --- | --- | --- | --- | --- |
| Shared reference UI | DONE | Align the dashboard, directories, forms, analytics and monitor with the approved light/green visual reference. | Shared CSS tokens and native table theme; responsive navigation; actual event chart and cached camera preview; compact profile headings; existing operator actions remain available. | Existing Streamlit UI and local preview API. | Five-page Streamlit smoke test; screenshots of all sections and monitor, desktop 1440 px and mobile 390 px; no document overflow; worker/API health probes passed on 2026-09-08. |
...
| Unknown visitors catalog | DONE | Group locally repeated unknown faces into reviewable visitor cards without changing ERP or RTSP processing. | Pending samples are clustered by a 0.5-CPU Docker service; operators can review photos, visits and history, correct groups, and create only local employees. Unknown data and all future local-employee events remain local. | PostgreSQL; retained local event photos; approved FaceID model. | 42 unit tests; Compose validation; all containers healthy; clusterer and worker health checks; live API card/detail check with `Asia/Tashkent` time; Streamlit employee-page test with zero exceptions on 2026-09-08. |
...
| 2026-09-08 | DONE | Added the local unknown-visitor catalog: conservative face grouping, camera/direction visit calculation, background 30-day backfill, retention cleanup, profile/history API and an operator UI for review, conversion, split and merge. The low-priority `unknown-clusterer` never opens RTSP and never sends unknown or converted-local records to ERP. | 42 unit tests including clustering, 10-minute visits, merge/split, conversion, backfill and retention; Compose validation; all services healthy; live worker/clusterer health checks; API list/detail with `Asia/Tashkent` timestamps; Streamlit employees page test with zero exceptions. |
| 2026-09-08 | DONE | Missing model weights now stop a camera process with one actionable message instead of a torch traceback, and the detector no longer falls back to an Ultralytics GitHub download on an Edge device. `main.py` validates detection and FaceID weights before starting any camera. The Docker image also selects the PyTorch CPU wheels that exist for the build architecture, so the CPU image builds on aarch64 hosts; the x86_64 `+cpu` pins are unchanged. | 46 unit tests including both missing-weight paths; container run exited 1 with the detector message and, with detector weights present, the FaceID pack message; `docker compose build` for every service; postgres/api/ui/unknown-clusterer healthy and edge-sync reporting an unconfigured ERP; five-page Streamlit AppTest with zero exceptions. Recognition was not verified end to end: the device has no approved weights or camera stream. |
