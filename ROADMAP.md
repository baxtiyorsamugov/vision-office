...
| ERP data sync | BLOCKED | Synchronize people, reference photos and 512-value embeddings through an adapter and ERP. | The approved production catalog was imported into local PostgreSQL: 17 active people, all named and recognition-ready. Compact hourly sync responses now preserve that cache. Production `/persons/sync` still omits `fio`, `erp_person_id` and photo URL, so it cannot automatically bootstrap or update metadata for future people. | Foundation; production sync payload with identity metadata. | 28 unit tests; live 17-person catalog bootstrap; live compact-sync preservation; worker restart loaded 17 embeddings on 2026-09-07. |
| Local-only employees | DONE | Allow optional people and photos stored only on the Edge device, with local attendance and no ERP delivery. | ERP and local profiles load into one FaceID cache. Local identities are stored as `local:<id>`; only they write `attendance`, and the ERP outbox rejects them. The Registration and Employees pages distinguish the two catalogs. | PostgreSQL local cache; recognition event pipeline. | 31 unit tests; Docker rebuild; worker loaded 17 ERP + 4 local embeddings; PostgreSQL outbox query found zero local IDs; dashboard smoke check on 2026-09-07. |
